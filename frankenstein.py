...
with g.as_default():
    ...
    with tf.Session() as sess:
        ...
        while tally < 100000000:
            ...
            length = len(g_v)
            percent =  int(length * FLAGS.dropout_rate)
            subset = sorted(random.sample(range(0,length), percent))
            g_i_new = [g_i[i] for i in subset]
            g_v_new = [g_v[i] for i in subset]
            a=sess.run(update_op, {#eta: lr,
                    local_gradient_indices: g_i_new, local_gradient: g_v_new})
